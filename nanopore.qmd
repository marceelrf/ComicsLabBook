...
`NanoPlot`[@coster2023] is a versatile tool for analyzing the quality of ONT sequencing data. It can be applied to fastq and bam files using the `--fastq` or `--bam` tag. The standard usage is:
...
The `--ubam` tag is useful for analyzing data before genome mapping.

#### Chopper

If you are working with fastq files, `Chopper` [@coster2023a] is a great option. You can select the min quality (-q), min length (-l), and other interesting features. The standard usage is:

```{bash}
#| eval: false

# Min quality = 20 and min read length as 1000
